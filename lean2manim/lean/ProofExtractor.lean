...
/-- The `#extract_proof` command.
    Full automatic proof interception requires lake integration (see README).
    When used standalone this command emits a skeleton JSON trace to the
    configured output path so that lean2manim can be run immediately with
    a placeholder trace, while also logging the expected JSON structure.
-/
elab "#extract_proof" : command => do
  let outputPath := lean2manim.output.get (← getOptions)
  logInfo m!"lean2manim: #extract_proof invoked (output → {outputPath}).
Note: full tactic-level interception requires running ProofExtractor inside a
lake build. The JSON proof trace must be generated by the `recordStep` hook.
See lean2manim/README.md §'Extracting a proof from Lean 4' for setup instructions.
A minimal skeleton JSON has been written to {outputPath} as a starting point."
  -- Write a minimal skeleton JSON so lean2manim CLI can process the file immediately
  let skeleton := "{\"theorem_name\":\"unknown\",\"theorem_statement\":\"\",\"steps\":[]}"
  IO.FS.writeFile outputPath skeleton
