...
from .proof_tree import ProofTree, TacticStep, GoalState
from .colors import expr_color, var_color
from .visual_dispatch import goal_visual_spec, tactic_anim_hint, _expr_label
...
def _safe_label(label: str) -> str:
    """Escape backslashes for use in Python string literals."""
...
def _node_var(node_id: str) -> str:
    ...
    return "node_" + node_id.replace("-", "_").replace(".", "_").replace(" ", "_")
...
def emit_scene_file(proof_tree: ProofTree, output_path: str) -> None:
    ...
    if proof_tree.steps and proof_tree.steps[0].pre_goals:
        ...
        if first_goal.id not in initial_goals_emitted:
            ...
            spec = goal_visual_spec(first_goal, color)
        ...
        if anim == "fly_in":
            ...
            for pg in step.post_goals:
                if pg.id not in emitted_goals:
                    ...
                    pg_spec = goal_visual_spec(pg, pg_color)
            ...
            if active_pre_goal_id:
                ...
                buf.write(f'{ind}self.play(Flash(active_nodes["{active_pre_goal_id}"], color="{expr_color(step.pre_goals[0].conclusion.identity_hash if step.pre_goals else "x")}"), run_time=0.5)\n')
